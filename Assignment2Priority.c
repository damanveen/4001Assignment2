			...
			//Else there already exists a thread in the list
				...
				if(iter->next == NULL){
					...
					for(iter; iter != NULL; iter = iter->next){
						if(NewItem->priority > iter->priority && (NewItem->priority < iter->next->priority || iter->next == NULL)){
